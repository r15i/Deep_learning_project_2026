...
In recent years, the rapid growth of the Internet of Things (IoT) has driven the development of innovative wireless systems and methodologies. Within healthcare, these wireless devices play a crucial role in monitoring vital parameters \cite{ubibreath-2015}. A key research focus is Human Activity Recognition (HAR), which leverages the Doppler shifts in Wi-Fi signals to detect human presence and movement. Implementing these systems is particularly valuable for assisted-living applications in healthcare environments \cite{MeneghelloNN-2023}.

Since the HAR is a classification problem, a deep-learning architecture like a ResNet \cite{He-2015} can be useful for this kind of task. Resnet address the optimization challenges of deep neural topologies by incorporating identity shortcut connections that mitigate vanishing gradients during backpropagation. In Human Activity Recognition (HAR) domains, standard feedforward architectures often struggle with raw sensor signals. The deployment of modified 1D-ResNet frameworks \cite{MDPI2025} effectively replaces complex manual feature engineering with hierarchical, end-to-end spatial-temporal feature extraction directly from multivariate time-series data. Furthermore, academic literature highlights the efficacy of integrating residual topologies with recurrent architectures, such as Bidirectional Long Short-Term Memory (BiLSTM) modules \cite{MDPI2022}, or multi-scale filter paradigms, such as Inception blocks \cite{IEEE2021}. These hybrid models seamlessly capture long-range temporal dependencies and fine-grained movement dynamics across complex action sets. Consequently, ResNet-based paradigms ensure stable convergence, enhance classification fidelity, and provide robust generalization across diverse.

In this report, we evaluate the performance of a customized Resnet on a dataset for a HAR system. The analysis is conducted using the dataset described in \cite{MeneghelloDataset-2023} and \cite{MeneghelloNN-2023}. To assess the model's performance, we utilize several metrics, including loss value, F1-score, and accuracy.
During the training phase we train the model only on the following labels: walking (W), running (R), jumping (J), sitting (L) and empty room (E).\\
The structure of the rest of the report is the following one:
\begin{enumerate}
    \item \textbf{Related work}: in Section \ref{sec:related_work} we describe the main paper useful to describe the SoA of the Resnet and its applications on HAR system;
    
    \item \textbf{Processing pipeline}: in Section \ref{sec:processing_architecture} we describe how the data are processed from the raw data up to the classification task;
    
    \item \textbf{Learning framework}: in Section \ref{sec:learning_framework} we describe the structure of the Resnet;

    \item \textbf{Results}: in Section \ref{sec:results} we show the results of the analysis.
\end{enumerate}

The work was divided in the following way: both the authors of this report defined the architectures of both neural networks and the processing pipeline. Davide Pimazzoni was responsible for finding articles regarding the architectures under examination; on the other hand Emilio Risi, on the other hand, handled the data collection and dataset creation.
